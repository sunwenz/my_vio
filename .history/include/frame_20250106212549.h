...
public:
    ...
    // void featuresDetection(const cv::Mat& mask = cv::Mat(ROW, COL, CV_8UC1, cv::Scalar(255))) const;

    std::unordered_map<ulong, cv::Point2f> features() const;
...
private:
    ...
    cv::Mat img_, img_right_;
    SE3 pose_;

    bool is_keyframe_;

    std::vector<cv::Point2f> keypoints_, keypoints_right_;
