...
struct GNSSData {
    ...
    GNSSData enu(const Eigen::Vector3d &origin){
        Eigen::Vector3d temp = blh_;
        blh_[0] = temp[1];
        blh_[1] = temp[0];
        Eigen::Matrix3d Dr = DR(origin);
...
struct ImuData
{
    ...
    ImuData() = default;
    ImuData(double t, const Eigen::Vector3d& gyro, const Eigen::Vector3d& accel) 
