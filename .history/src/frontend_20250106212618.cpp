...
void Frontend::featureDetection(Frame::Ptr frame){
    cv::goodFeaturesToTrack(frame->imgLeft(), keypoints_curr_, MAX_FEATURES, 0.01, MIN_DIST, mask_);
}

void Frontend::setMask(){
    mask_ = cv::Mat(camera_->height(), camera_->width(), CV_8UC1, cv::Scalar(255));
    for(const auto &feature : current_frame_->features()){
        cv::circle(mask_, feature.second.first, 10, 0, -1);
    }
}
