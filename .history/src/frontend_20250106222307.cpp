...
Status Frontend::trackFrame(Frame::Ptr frame){
    ...
    // track from left to right
    {
        ...
        std::vector<cv::Point2f> keypoints_reverse;
        std::vector<uchar> status_l2r, status_r2l;
        std::vector<float> err;
        cv::calcOpticalFlowPyrLK(current_frame_->imgLeft(), current_frame_->imgRight(), keypoints_curr_, keypoints_curr_right_, status_l2r, err);
        cv::calcOpticalFlowPyrLK(current_frame_->imgLeft(), current_frame_->imgRight(), keypoints_curr_right_, keypoints_reverse, status_r2l, err);
        for(size_t i = 0; i < status_l2r.size(); i++){
            if(status_l2r[i] && status_r2l[i] && calPixelDistance(keypoints_curr_[i], keypoints_reverse[i]) < 1.0 && inBorder(keypoints_curr_right_[i])){
                status_l2r[i] = 1;
            }else{
                status_l2r[i] = 0;
            }
        }
        reduceVector(keypoints_curr_right_, status_l2r);
        reduceVector(keypoints_curr_, status_l2r);
        LOG(INFO) << "TRACK L2R" << keypoints_curr_right_.size() << " FEATURES, TOOK " << timer.toc() << " ms.";
    }
    return track_status;
}
